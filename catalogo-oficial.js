...
(function(){
  ...
  let tipoAtual='Tecnólogo', mostrarTodos=false;

  function iniciar(){
    const grid=document.getElementById('catalogGrid'), count=document.getElementById('catalogCount'), search=document.getElementById('catalogSearch'), area=document.getElementById('catalogArea'), more=document.getElementById('catalogMore');
    if(!grid||!count)return;

    // Remove os listeners do catálogo antigo para que apenas a lista oficial controle as abas.
    let tabs=[...document.querySelectorAll('.catalog-tab')];
    tabs.forEach(tab=>{const clone=tab.cloneNode(true);tab.replaceWith(clone);});
    ...
    function render(){
      const termo=(search?.value||'').trim().toLocaleLowerCase('pt-BR'), areaEscolhida=area?.value||'';
      const lista=cursos.filter(c=>c.tipo===tipoAtual&&(!areaEscolhida||c.area===areaEscolhida)&&(!termo||c.nome.toLocaleLowerCase('pt-BR').includes(termo)||c.area.toLocaleLowerCase('pt-BR').includes(termo)));
      const exibidos=(mostrarTodos?lista:lista.slice(0,9));
    ...
    tabs.forEach(tab=>tab.addEventListener('click',()=>{
      ...
      tipoAtual=tipo==='Tecnólogo'||tipo==='tecnologo'?'Tecnólogo':tipo==='Bacharelado'||tipo==='bacharelado'?'Bacharelado':'Licenciatura';
      mostrarTodos=false;
    ...
    estilo.textContent=`.catalog-card h3{font-size:16px;line-height:1.35;color:var(--dark);margin:12px 0 7px}.catalog-area-label{font-size:11px;color:var(--muted);font-weight:700;margin-bottom:16px}.catalog-meta{display:flex;justify-content:space-between;align-items:center;border-top:1px solid var(--border);padding-top:12px;margin-top:auto;margin-bottom:15px;font-size:11px;color:var(--muted)}.catalog-meta strong{color:var(--purple);font-size:12px}.catalog-course-btn{width:100%;padding:10px 12px;font-size:12px}.catalog-empty{grid-column:1/-1;text-align:center;padding:35px;color:var(--muted);font-weight:700}@media(max-width:900px){.catalog-grid{grid-template-columns:repeat(2,1fr)}}@media(max-width:600px){.catalog-grid{grid-template-columns:1fr}.catalog-toolbar{grid-template-columns:1fr}.catalog-wrap{padding:22px}}`;
